...
class Reg(object):
    ...
    def __exit__(self, None, None, None):
        self.finish()
...
class ThreadedFunc(ThreadedStream):
    ...
    def run(self):
        result = self.func(*self.args, **self.keys)
        if not util.is_generator(func):
...
def threaded(func, *args, **keys):
    return ThreadedFunc(func, *args, **keys)

def wait():
    pass

def wait_any(*sources):
    condition = threading.Condition
    sources = set(sources)

    def waiter(source, final, success, value):
        if not final:
            source.register(waiter)
        else:
